...
public class WheelHandler : MonoBehaviour
{
    ...
    public void ApplyTorque(Vector2 moveInput, float stopInput)
    {
        ...
        if (rayDidHit)
        {
            ...
            ApplySteering();
            if (_stopInput > 0f || _moveInput.y == 0f)
            {
                BrakeTorque();
            }
            else
    ...
    private void ApplyAcceleration()
    {
        ...
        float availableTorque = carStats.powerCurve.Evaluate(normalizedSpeed) * _moveInput.y;
    ...
    private float CalculateYawAngle()
    {
        ...
        if (_moveInput.x > 0f && t <= 1f)
        {
            t += carStats.turningSpeed * Time.fixedDeltaTime;

        }
        else if (_moveInput.x < 0f && t >= -1f)
        {
            t -= carStats.turningSpeed * Time.fixedDeltaTime;
        }
        else
